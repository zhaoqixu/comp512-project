...
public class LockManager
{
    ...
    private static int DEADLOCK_TIMEOUT = 10000000;
        ...
        try {
            ...
            while (bConflict) {
                synchronized (this.lockTable) {
                    ...
                    // System.out.println("BitSet: " + bConvert);
                    if (!bConflict) {
                        // no lock conflict
                        ...
                        if (bConvert.get(0) == true) {
                            ...
                            ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                            // two objects in lock table for easy lookup.
                            TrxnObj trxnObj1 = new TrxnObj(xid, strData, TrxnObj.READ);
                            DataObj dataObj1 = new DataObj(xid, strData, DataObj.READ);
                            this.lockTable.remove(trxnObj1);
                            this.lockTable.remove(dataObj1);
                            this.lockTable.add(trxnObj);
                            this.lockTable.add(dataObj);
                            System.out.println("Lock Converted from READ to WRITE");
                        } else {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (dataObj.getXId() == dataObj2.getXId()) {    
                ...
                if (dataObj.getLockType() == DataObj.READ) {    
                    ...
                    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                    // (1) transaction already had a READ lock
                    // need to check if there are other XId holding any READ lock
                    // no need to check WRITE locks, since no one holds WRITE lock if requsting XId has READ lock already
                    if (dataObj2.getLockType() == DataObj.READ) {
                        for (int j = 0; j < size; j++) {
                            dataObj3 = (DataObj) vect.elementAt(j);
                            if (dataObj3.getXId() != dataObj2.getXId()) {
                                System.out.println("Held READ, want WRITE, someone has READ");
                                return true;
                            }
                        }
                        bitset.set(0);
                        // System.out.println("bitset changed");
                    }
                    // (2) transaction already had a WRITE lock, then this is a redundant WRITE lock request
